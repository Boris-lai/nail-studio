...
serve(async (req) => {
  ...
  const SUPABASE_URL = Deno.env.get("SUPABASE_URL") ?? Deno.env.get("VITE_SUPABASE_URL"); // Fallback
  const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get("SERVICE_ROLE_KEY");

  // Hardcode to avoid dynamic origin issues
  const REDIRECT_URI = "https://qkgglpyddnmyhoybssye.supabase.co/functions/v1/line-auth?action=callback";
  ...
  if (action === "callback") {
    ...
    try {
      ...
      */
      ...
      if (linkError) throw new Error("Failed to generate login link: " + linkError.message);
